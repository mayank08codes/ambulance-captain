...
function contextualHospital(hospital: HospitalOption, position: Coordinates) {
  const offsets: Record<string, [number, number]> = {
    ...
    "MetroCare Emergency Institute": [0.009, 0.018],
  };
...
function App() {
  ...
  const selectHospital = (hospital: HospitalOption) => {
    ...
    if (tripStage === "enroute") {
      setSection("trip");
      notify(`Route updated to ${hospital.name}. Ambulance destination changed to its ${hospital.erEntrance}.`);
    } else {
      notify(`${hospital.name} selected as the next hospital destination.`);
    }
  };
