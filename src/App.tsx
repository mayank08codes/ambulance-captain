...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><Ambulance size={22} /></div><div><strong>RescueRoute</strong><span>Driver operations</span></div></div>
...
function Login({ onLogin }: { onLogin: () => void }) {
  ...
  return <div className="login-shell"><div className="login-visual"><div className="login-brand"><div className="brand-mark"><img src="/manus-storage/rescueroute-ambulance-logo_3da843c9.png" alt="RescueRoute logo" /></div><div><strong>RescueRoute</strong><span>Emergency response operations</span></div></div><div className="login-visual-copy"><span className="eyebrow">RESCUEROUTE · DRIVER PORTAL</span><h1>Every request. Every route. Care at the right hospital.</h1><p>Accept ambulance requests, verify passengers, and navigate to the right hospital from one focused operations workspace.</p><div className="login-proof login-trust-row"><div><ShieldCheck size={17} /><span>Verified driver access</span></div><div><Navigation size={17} /><span>Live trip guidance</span></div><div><Hospital size={17} /><span>Hospital-aware routing</span></div></div></div><div className="login-emergency"><ShieldCheck size={18} /><span>Secure driver access · Built for emergency response</span></div></div><div className="login-card"><div className="login-card-head"><div className="login-icon"><img src="/manus-storage/rescueroute-ambulance-logo_3da843c9.png" alt="RescueRoute logo" /></div><span className="eyebrow">WELCOME BACK</span><h2>Sign in to Captain</h2><p className="muted">Use your registered driver details to continue.</p></div><label className="field-label">Phone number</label><input className="text-field" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+91 98765 42041" autoComplete="tel" /><label className="field-label">Password</label><input className="text-field" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter password" autoComplete="current-password" /><div className="login-row"><label className="remember"><input type="checkbox" defaultChecked /> Keep me signed in</label><button className="text-link" onClick={() => setError("Password reset is available through dispatcher support.")}>Forgot password?</button></div>{error && <div className="login-error"><X size={15} /> {error}</div>}<button className="primary-button full login-submit" onClick={submit} disabled={busy}>{busy ? "Signing you in…" : <><ArrowRight size={17} /> Sign in</>}</button><p className="login-footnote">Demo access: any valid phone and a 6+ character password.</p></div></div>;
}
