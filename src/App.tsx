...
const hospitals: HospitalOption[] = [
  { name: "Central Emergency Hospital", rating: 4.8, reviews: 1240, beds: "ER available", capacity: 82, speciality: "Emergency, trauma, ICU", emergencyLevel: "Level 1 trauma", openNow: true, address: "Emergency District · Main Avenue", phone: "+1 212 410 2200", erEntrance: "Ambulance Dock A · Emergency Bay", erLocation: { lat: 40.7356, lng: -73.9794 }, tag: "Best overall", feedback: "Fast triage and consistently calm emergency teams", reviewHighlight: "Patients praise short intake times and clear updates", location: { lat: 40.735, lng: -73.98 } },
  { name: "Riverside Medical Center", rating: 4.6, reviews: 864, beds: "Cardiac unit", capacity: 64, speciality: "Cardiac, emergency, NICU", emergencyLevel: "Level 2 trauma", openNow: true, address: "22 Riverside Drive · Medical Quarter", phone: "+1 212 433 8800", erEntrance: "East ER Ramp · Cardiac Intake", erLocation: { lat: 40.7315, lng: -73.9884 }, tag: "Top rated", feedback: "Strong cardiac response and family communication", reviewHighlight: "Feedback highlights attentive nurses and clean facilities", location: { lat: 40.731, lng: -73.989 } },
  { name: "Northpoint General Hospital", rating: 4.4, reviews: 702, beds: "Trauma centre", capacity: 46, speciality: "Trauma, orthopaedics, ER", emergencyLevel: "Level 1 trauma", openNow: true, address: "8 Northpoint Road · Civic Medical Zone", phone: "+1 212 455 7711", erEntrance: "Trauma Gate 2 · Ambulance Ramp", erLocation: { lat: 40.6995, lng: -74.0114 }, tag: "24/7 intake", feedback: "Reliable trauma intake with specialist coverage", reviewHighlight: "Drivers report dependable handover and 24/7 reception", location: { lat: 40.699, lng: -74.012 } },
];
...
function App() {
  ...
  useEffect(() => {
    if (!audioPingSeconds || audioPingSeconds >= 5) return;
    const timer = window.setInterval(() => setAudioPingSeconds((seconds) => {
      const next = Math.min(seconds + 1, 5);
      if (next === 5) setAudioVerified(true);
      return next;
    }), 1000);
    return () => window.clearInterval(timer);
  }, [audioPingSeconds]);
